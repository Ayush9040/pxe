                        ...
                        <div class="col-md-10 gd-text-sm-center">
                            <div class="sptfl">
                                <div class="quickFilter">
                                    <h4 class="quickFilter-title uppercase" style="font-weight:700"><i class="fas fa-filter"></i>{{__('Quick filter')}}</h4>
                                    <ul id="quick_filter">
                                        <li><a datahref=""  class="uppercase"><i class="icon-chevron-right pr-2 "></i>{{__('All products')}} </a></li>
                                        <li class=""><a href="javascript:;" class="uppercase" data-href="feature"><i class="icon-chevron-right pr-2"></i>{{__('Featured products')}} </a></li>
                                        <li class=""><a href="javascript:;" class="uppercase" data-href="best"><i class="icon-chevron-right pr-2"></i>{{__('Best sellers')}} </a></li>
                                        <li class=""><a href="javascript:;" class="uppercase" data-href="top"><i class="icon-chevron-right pr-2"></i>{{__('Top rated')}} </a></li>
                                        <li class=""><a href="javascript:;" class="uppercase" data-href="new"><i class="icon-chevron-right pr-2"></i>{{__('New Arrival')}} </a></li>
                                        <hr class="mt-2" style="border:1px solid #000"/>
                                        <li class="pt-3">
                                           @if ($setting->is_attribute_search == 1)
                                             @foreach ($attrubutes as $attrubute)
                                             <h3 class="widget-title uppercase">{{ __('Filter by') }} {{$attrubute->name}}</h3>
                                             <section class="rounded">
                                               ...
                                               <div class="custom-control custom-checkbox">
                                                 ...
                                                 <label class="custom-control-label uppercase" for="{{$attrubute->id}}{{$option->name}}" style="color:#505050 !important">{{$option->name}}<span class="text-muted" ></span></label>
                                             </div> 
                                               @endif
                                               @endforeach
                                             </section>
                                        ...
                                        <li class="pt-2">
                                            ...
                                            <form class="price-range-slider" method="post" data-start-min="{{request()->input('minPrice') ? request()->input('minPrice') : '0'}}" data-start-max="{{request()->input('maxPrice') ? request()->input('maxPrice') : $setting->max_price}}" data-min="0" data-max="{{$setting->max_price}}" data-step="5">
                                                ...
                                                <div class="column">
                                                    <button class="btn btn-primary btn-sm uppercase" id="price_filter" type="button"><span>{{__('Filter')}}</span></button>
                                                </div>
                                                <div class="column">
                                                    <div class="ui-range-values">
                                                    <div class="ui-range-value-min uppercase"  style="color:#505050 !important">{{PriceHelper::setCurrencySign()}}<span class="min_price"></span>
                                                        <input type="hidden">
                                                    </div>-
                                                    <div class="ui-range-value-max uppercase"  style="color:#505050 !important">{{PriceHelper::setCurrencySign()}}<span class="max_price"></span>
                                                        <input type="hidden">
                                                    </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="shop-sorting">
                                    <label for="sorting" class="uppercase">{{__('Sort by')}}:</label>
                                    <select class="form-control uppercase" id="sorting" style="font-weight:700">
                                    <option value="" class="uppercase">{{__('All Products')}}</option>
                                    <option value="low_to_high" class="uppercase" {{request()->input('low_to_high') ? 'selected' : ''}}>{{__('Low - High Price')}}</option>
                                    <option value="high_to_low" class="uppercase" {{request()->input('high_to_low') ? 'selected' : ''}}>{{__('High - Low Price')}}</option>
                                    </select>
