...
class HomePageController extends Controller
{
    ...
    {
       //$datas= Exclusivedeal::orderBy('id','desc')->get();
       //dd($datas);
        return view('back.exclusive.index',[
            'datas' => Exclusivedeal::orderBy('id','desc')->get()
        ]);
    }
    public function exclusivecreate()
    {
        return view('back.exclusive.create',[
            'datas' => Exclusivedeal::orderBy('id','desc')->get()
        ]);
    }
    public function exclusivestore(Request $request)
    {
        ...
        $input = $request->all();
        $input['image'] = ImageHelper::handleUploadedImage($request->file('image'),'assets/images');

        $input['video'] = ImageHelper::handleUploadedImage($request->file('video'),'assets/videos');
        $input['title']=$request->title;
        Exclusivedeal::create($input);

        return redirect()->route('back.exclusive.index')->withSuccess(__('New Exclusive Added Successfully.'));
    }
    /**
     ...
     */
    public function exclusivedestroy($id)
    {
       // dd($exclusivedeal);
       // ImageHelper::handleDeletedImage($exclusivedeal,'image','assets/images/');
       // ImageHelper::handleDeletedImage($exclusivedeal,'video','assets/videos/');
        Exclusivedeal::destroy($id);
       

        return redirect()->route('back.exclusive.index')->withSuccess(__('Exclusive Deleted Successfully.'));
    }
